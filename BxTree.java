...
public class BxTree<K extends Comparable<? super K>, V> implements Serializable {
	...
	public BxTree(int branchingFactor) {
		...
		while (!queue.isEmpty()) {
			...
			while (!queue.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (it.hasNext())
						sb.append(", ");
					if (node instanceof BxTree.InternalNode)
	...
	private class LeafNode extends Node {
		...
		List<V> getRange(K key1, RangePolicy policy1, K key2,
			...
			while (node != null) {
				...
				Iterator<List<V>> vIt = node.values.iterator();
				while (kIt.hasNext()) {
					K key = kIt.next();
					List<V> value = vIt.next();
